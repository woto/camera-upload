...
| `POST` | `/uploads/{id}/exports` | создать сохранённую версию прокси |
| `GET` | `/uploads/{id}/exports/{exportId}` | одна версия прокси и её motion analysis |
| `PUT` | `/uploads/{id}/exports/{exportId}` | обновить версию; при изменении канонических настроек возвращает запись с новым `exportId` |
...
тело ограничено 2 МиБ и строго валидируется. Допустимые верхние границы параметров
совпадают с camera-motion: `enter <= 100`, `settle <= 100`,
`settle_samples <= 1000`, `min_segment <= 86400`, `features <= 100000` и
`min_inliers <= 100000`. Изменение канонических настроек версии атомарно заменяет
запись: создаётся новый `exportId`, а прежний `analysis` удаляется. Старый
detail/proxy/analysis URL после этого отвечает `404`, поэтому клиенты
`PUT /uploads/{id}/exports/{exportId}` обязаны использовать запись, возвращённую
в ответе. Повторное сохранение тех же канонических настроек сохраняет прежние
`exportId` и `analysis`. Ротация идентификатора на каждом реальном изменении
исключает доставку результата старой попытки после цикла S1→S2→S1.
