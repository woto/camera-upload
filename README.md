...
версии атомарно заменяет запись: создаётся новый `exportId`, а прежний `analysis`
удаляется. Старый detail/proxy/analysis URL после этого отвечает `404`, поэтому
клиенты `PUT /uploads/{id}/exports/{exportId}` обязаны использовать запись,
возвращённую в ответе. Повторное сохранение тех же канонических настроек сохраняет
прежние `exportId` и `analysis`. Ротация идентификатора на каждом реальном
изменении исключает доставку результата старой попытки после цикла S1→S2→S1.
